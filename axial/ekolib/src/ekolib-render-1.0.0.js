...
function renderPreview(contest, divselector){
    // replaces
    $(divselector).html('');
    $(divselector).append(EkoMakePreview(contest));
    $(divselector).find(' .steps').tabs();
}
...
function EkoMakePreview(contest){
    ...
    if(contest.description_included)
        contestheaderElt.append($('<span></span>').text(contest.description));
    contestElt.append(contestheaderElt);
    
    
    var tabsElt = $('<ul class="tabs"></ul>');
    ...
    for (var s=0;s<contest.steps.length;s++ ){
        ...
        for (var f=0;f<step.fields.length;f++ ){
            ...
            var fieldElt = $('<div/>');
            var labelElt = $('<span class="'+(field.type=='LABEL' ? "fieldLabelLabel" : "fieldLabel")+'"/>');
            ...
            if(field.type=='LABEL'){
                fieldElt.append($('<div class="field">').append(labelElt));
            }else{
        ...
        if (s<(contest.steps.length-1)){
            ...
            var postBtn = EkoButton(contest.submit_translation);
...
function vote(response){
    $.ajax({
        type:"POST",
        url: "accountcontestservices.asmx/Vote",
        data: {
...
function EkoMakeInput(field,boundDict, propertyName, validatorFeedbackCallback){
    ...
    var onchangeTextcallback=function(){
        ...
        //boundDict["queryString"] = (boundDict["queryString"] ? boundDict["queryString"] + '&' : '' ) +  field.subtype + "=" + encodeURIComponent(propertyVal);
    ...
    if (field.type=='EKO') {
        ...
        if (field.subtype=='shorttext') {
            ...
            inputElt.change(onchangeTextcallback);
            return inputElt;
        } else if (field.subtype=='hiddentext') {
            
            var inputElt = $('<input class="field-shorttext" id="'+field.label+'" type="text"></input>');
            inputElt.attr("name", field.label);
            inputElt.change(onchangeTextcallback);
            return inputElt;
        }
    ...
    } else if (field.type=='CHOICESET') {
        debug('Choice set of subtype: '+field.subtype);
        //var cs = eko.contest.FieldTypes.CHOICESET[field.subtype];
        var cs = eko.groupsets[field.subtype];
        var divElt = makeChainedSelect(cs);
        return divElt;
    }
}
