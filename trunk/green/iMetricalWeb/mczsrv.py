...
def create_handler(zip_files, max_age=None, public=None):
  ...
  class HandlerWrapper(MemcachedZipHandler):
    ...
    def get(self, name):
      if name=='fr':
        logging.error('trapped fr: redirecting')
        self.redirect("%s%s" % (name, '/'))
        return
    ...
    if resp_data is None:
      ...
      if resp_data is None:
        ...
        if resp_data is not None:
          ...
          logging.error('Adding %s to negative cache, serving 404', name)
...
#      return name
  ...
  def GetFromStore(self, file_path):
    ...
    logging.error('path: %s should be in %s', file_path, archive_name)
    if not archive_name:
      archive_name = file_itr.next()[0]
    
    while resp_data is None and archive_name:
      logging.error('   try %s',archive_name)
      ...
      if zip_archive:
        ...
        # with that
        try:
          #logging.error('   content: %s',zip_archive.namelist())
          resp_data = zip_archive.read(file_path)
        except (KeyError, RuntimeError), err:
          # no op
          x = False
        if resp_data is not None:
          logging.error('%s read from %s', file_path, archive_name)
  ...
  def LoadZipFile(self, zipfilename):
    ...
    if zip_archive is None:
      try:
        ...
        logging.error('   cachingorama: %s',zipfilename)
        #logging.error('     content: %s',zip_archive.namelist())
        for zzen in zip_archive.namelist():
          #logging.error('     entry: %s',zzen)
          if zzen[len(zzen) - 1:] == '/':
            logging.error('       dir entry: %s',zzen)
