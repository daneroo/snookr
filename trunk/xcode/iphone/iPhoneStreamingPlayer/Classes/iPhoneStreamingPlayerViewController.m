...
- (void)viewDidLoad
{
	...
	// web view now: from web...
	/*
	NSURL *url = [NSURL URLWithString:@"http://axial.imetrical.com/"];
	NSURLRequest *request = [NSURLRequest requestWithURL:url];
	[self.webView loadRequest:request];
	 */
	NSString *path = [[NSBundle mainBundle] pathForResource:@"lastCall" ofType:@"html" inDirectory:@"html"];
	...
	if (streamer.bitRate != 0.0)
	{
		...
		if (duration > 0)
		{
			[positionLabel setText:
				[NSString stringWithFormat:@"Time Played: %.1f/%.1f s.",
					progress,
					duration]];
...
- (void)webViewDidFinishLoad:(UIWebView *)aWebView
{
	...
	[aWebView stringByEvaluatingJavaScriptFromString:@"internalFunc('objC called Me!')"];
	 */
}	
...
- (BOOL)webView:(UIWebView *)aWebView 
	...
	if ([requestString hasPrefix:@"APP:PING"]) {
		NSLog(@"request PING: %@",requestString); 
		NSLog(@"Here is the audio:  %@",[aWebView stringByEvaluatingJavaScriptFromString:@"document.getElementById('audiourl').value"]);
		downloadSourceField.text = [aWebView stringByEvaluatingJavaScriptFromString:@"document.getElementById('audiourl').value"];
		[self buttonPressed:button];
		return NO;
	}	
	NSLog(@"request PASSTHRU: %@",requestString); 
